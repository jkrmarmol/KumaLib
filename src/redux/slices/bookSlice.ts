...
export const download = createAsyncThunk(
  ...
  async ({ title, extension, id, hash }: any) => {
    try {
      ...
      //     const base64 = await FileSystem.readAsStringAsync(downloadResponse.uri, { encoding: FileSystem.EncodingType.Base64 })

      //     await FileSystem.StorageAccessFramework.createFileAsync(permissions.directoryUri, title + '.' + extension, downloadResponse.headers['content-type'])
      //       .then(async (uri) => {
